...
3. Enforce singular platform-role permissions and strict OWNER/CASHIER place isolation.
...
An authenticated user with one or more active `PlaceMember(role=OWNER)` assignments. OWNER authority is place-scoped and independent from the user's platform role.

### Super administrator

An authenticated user whose `User.platformRole` is `SUPER_ADMIN`. A SUPER_ADMIN manages platform security, place memberships, moderation, and globally permitted operational data.

### Administrator

An authenticated user whose `User.platformRole` is `ADMIN`. ADMIN has the explicit global permissions defined by SRS v1.3 but cannot perform SUPER_ADMIN-only platform-role or OWNER-membership administration.

### Cashier

An authenticated user with an active `PlaceMember(role=CASHIER)` assignment. CASHIER may read tables and process orders only for assigned places and has no place-configuration authority.
...
- Singular platform-role administration by SUPER_ADMIN and place-scoped OWNER/CASHIER memberships.
...
2. The backend creates the user with `User.platformRole = USER`.
...
2. An authenticated OWNER or CASHIER member for that place retrieves the order.
...
- PR-AUTH-002: Every new active account is created with `User.platformRole = USER`.
...
- PR-RBAC-001: Every active user has one platform role and may independently hold memberships across multiple places.
- PR-RBAC-002: Only SUPER_ADMIN can change another user's platform role.
- PR-RBAC-003: The last active SUPER_ADMIN cannot be demoted or deactivated.
- PR-RBAC-004: Place authority is represented by OWNER/CASHIER memberships, and an active place must retain an active OWNER.
...
- Each place operates in its persisted IANA timezone.
...
- Cashiers authenticate with their own account and current `PlaceMember(role=CASHIER)` assignment.
- Product stakeholders must approve production retention and operational policies without changing the SRS v1.3 authorization matrix implicitly.

## 12. Risks and product decisions pending

- CASHIER membership onboarding and revocation must be operationally manageable without sharing OWNER credentials.
- Place-timezone changes must preserve correct open-hours evaluation.
